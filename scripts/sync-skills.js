...
const isDirectoryOrSymlink = entry => entry.isDirectory() || entry.isSymbolicLink()
...
export const syncSkills = (targetDir = defaultTargetDir) => {
  ...
  if (agentsStat.isSymbolicLink()) {
    throw new Error(
      "[sync-skills] .agents/skills must be a real directory, not a symlink."
    )
  }
  ...
  console.log(`[sync-skills] linked=${linked} removed=${removed} skipped=${skipped}`)
