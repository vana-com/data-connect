...
describe("buildUpdaterManifest", () => {
  it("builds a manifest with canonical macOS platform keys", () => {
  ...
  it("fails when alias normalization creates duplicate platform matches", () => {
    expect(() =>
      buildUpdaterManifest(
        {
          ...
          assets: [
            ...
            {
              name: "DataConnect_0.7.42_x86_64.app.tar.gz",
        ...
        new Map([
          ["DataConnect_0.7.42_x64.app.tar.gz.sig", "sig-x64"],
          ["DataConnect_0.7.42_x86_64.app.tar.gz.sig", "sig-x86_64"],
          ["DataConnect_0.7.42_aarch64.app.tar.gz.sig", "sig-aarch64"],
        ])
      )
    ).toThrow("Multiple updater assets matched darwin-x86_64")
  })

  it("fails when a required macOS target is missing", () => {
    expect(() =>
      buildUpdaterManifest(
        {
          tag_name: "v0.7.42",
        ...
        new Map([["DataConnect_0.7.42_aarch64.app.tar.gz.sig", "sig-aarch64"]])
      )
    ).toThrow("Missing updater asset for platform: darwin-x86_64")
  })
})
