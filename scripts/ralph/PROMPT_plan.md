0a. Study `__FEATURE_ROOT__/specs/*` with up to 250 parallel Sonnet subagents to
learn the application specifications.
0b. Study `__FEATURE_ROOT__/IMPLEMENTATION_PLAN.md` (if present) to understand
the plan so far.
...
`src/*` and compare it against `__FEATURE_ROOT__/specs/*`. Use an Opus subagent
to analyze findings, prioritize tasks, and create/update
`__FEATURE_ROOT__/IMPLEMENTATION_PLAN.md` as a bullet point list sorted in
...
inconsistent patterns. Study `__FEATURE_ROOT__/IMPLEMENTATION_PLAN.md` to
...
missing; confirm with code search first. Treat `src/lib` as the project's
...
ULTIMATE GOAL: We want to achieve the feature goal defined in
`__FEATURE_ROOT__/specs/*`. Consider missing elements and plan accordingly. If
an element is missing, search first to confirm it doesn't exist, then if needed
author the specification at `__FEATURE_ROOT__/specs/FILENAME.md`. If you create
a new element then document the plan to implement it in
`__FEATURE_ROOT__/IMPLEMENTATION_PLAN.md` using a subagent.
